...
**Maintenance Release: carries the charting terminal from tradeboard-charts 2.0.2 to 2.1.7, adding TPO and session volume profiles, a pane Objects panel and a data loading controller, while fixing replay isolation and stale history loads; completes the Upstox V3 migration and corrects tick size, daily-candle stamping and previous-close reporting across Upstox, Kotak, Samco, Groww and Flattrade; makes GTT history visible in the order book; and clears every open Dependabot advisory**
...
platform version moves from 2.0.2.3 to 2.0.2.4; the pinned `tradeboard` SDK moves
...
* `tradeboard-charts`: **2.0.2** to **2.1.7**
* The pinned `tradeboard` SDK: **2.0.3** to **2.0.5** (`4cb25db21`, `ddc8cb57a`)
