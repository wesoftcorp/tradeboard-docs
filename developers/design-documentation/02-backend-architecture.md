...
Tradeboard uses Flask 3.1 with Flask-RESTX, Flask-SocketIO, Flask-Limiter, Flask-WTF CSRF, SQLAlchemy, APScheduler, DuckDB, ZeroMQ, and a separate asyncio WebSocket proxy. Python `>=3.12` is required.
...
|---|---|---|
| REST v1 | `restx_api/__init__.py`, prefix `/api/v1` | Tradeboard API key; CSRF exempt |
