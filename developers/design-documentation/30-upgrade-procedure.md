...
The migration runner executes an ordered, idempotent list of targeted scripts. Database modules also use idempotent table initialization during application startup. Tradeboard does not expose a `database.init_all_databases()` helper.
...
The current updater's automatic loop copies `tradeboard.db`, `logs.db`, `latency.db`, `sandbox.db`, and `historify.duckdb`; it does not include `health.db`. Copy `health.db` separately when retaining health history matters. This limitation should remain explicit until the updater is changed.
