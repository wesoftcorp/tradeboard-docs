...
Tradeboard has two MCP deployment modes:

| Transport | Entry | Registration | Auth |
|---|---|---|---|
| Local stdio | `mcp/mcpserver.py` | Spawned by a desktop/client config | Tradeboard API key and host passed to local process |
| Remote HTTP/SSE | `blueprints/mcp_http.py` | Opt-in during Flask startup | OAuth 2.1 access token and scopes |

Both modes expose trading and research tools backed by Tradeboard services/SDK behavior. The Python application currently pins the Tradeboard SDK at `2.0.3`.

## Local Stdio

The MCP client launches `.venv/bin/python mcp/mcpserver.py <api-key> <host>` (Windows uses the equivalent interpreter path). It communicates over stdio and calls the configured Tradeboard host. The API key should be scoped to the local self-hosted installation and protected as a secret in client configuration.
...
hosted MCP client
  ...
  -> Tradeboard service/API behavior
```
