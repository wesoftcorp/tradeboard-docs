...
8. **Read-only mode**: `TRADEBOARD_MCP_READ_ONLY` removes every order tool from the registry, so even a client already holding `write:orders` cannot place an order
